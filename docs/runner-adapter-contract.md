...
| `claude_code` | `scripts/runner_adapters/claude_code.py` | **Implemented, `supports_auto_execute="semi_auto"`, `consumes_paid_credits=True` (OP-ClaudeAdapter-001, budget-gated since OP-ClaudeBudgetGate-001).** Verified against the real `claude` CLI (v2.1.199), not guessed: `execute()` genuinely invokes `claude --print --output-format json` via `subprocess.run()` with an argument list (never `shell=True`), maps the work order's `allowed_actions` onto `--allowedTools`/`--disallowedTools`, and parses the embedded result JSON out of the CLI's JSON-wrapper response. "semi_auto" rather than "yes" because it depends on a one-time, out-of-adapter-control step — the workspace must already be interactively trusted by Claude Code, or every call fails (detected and reported clearly, not silently degraded). Never passes `--dangerously-skip-permissions` to work around this. Because `consumes_paid_credits=True`, `run_work_order.py` refuses to call `execute()` at all without an explicit `--max-budget-usd`/`COMMANDPILOT_CLAUDE_MAX_BUDGET_USD` — that value (or, absent one, the work order's own `approval_scope.max_cost_usd`) is what actually reaches `--max-budget-usd` on the CLI call. See `docs/background-dev-team-runbook.md`'s Fast Path §B2. Also implements `execute_step()` (`supports_step_execution=True`) — see `--per-step` below. |
| `claude_code_sandboxed` | `scripts/runner_adapters/claude_code_sandboxed.py` | **Implemented, `supports_auto_execute="yes"`, `safety_level="sandboxed"`, `consumes_paid_credits=True`.** The same `claude` CLI, run fully unattended (`--dangerously-skip-permissions`) inside a disposable Docker container against a disposable `git worktree` — never the real working tree. `--allowedTools`/`--disallowedTools` are still passed (belt-and-suspenders even without a permission prompt to answer). Changes land as a `type: "diff"` artifact on the work order (extracted from the worktree on the host after the container exits), never applied to the real repo automatically. Requires Docker (`docker build -t commandpilot-sandbox:latest scripts/sandbox` once). Mocked-test-verified (`scripts/test_sandbox_adapter.py`) and the Docker image itself builds successfully; **not yet verified with a real live container run against a real work order** — see CLAUDE.md's "Bekannte Risiken". See the adapter's own module docstring for the full safety argument. |
...
**`--per-step` (`supports_step_execution`).** Orthogonal to the table above
— any adapter can, in principle, implement `execute_step()` (single
ticket-plan step in, that step's result out) alongside `execute()` (whole
order in one call). Currently `claude_code` and `claude_code_sandboxed`
both do; `manual_prompt`/`codex`/`openclaw` don't. `run_work_order.py
--mode execute --per-step` drives `execute_step()` once per step, in
order, writing each step's result back immediately instead of waiting for
one final whole-order result — see `_run_step_by_step()` and
`build_step_prompt()` (the single-step sibling of `build_runner_prompt()`,
in `base.py`).
